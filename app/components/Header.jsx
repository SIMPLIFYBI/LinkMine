...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-slate-900/70 backdrop-blur border-b border-white/10 pt-[env(safe-area-inset-top)]">
      <div className="mx-auto max-w-screen-xl px-4 h-14 flex items-center md:justify-between justify-center relative">
        {/* Centered on mobile, normal flow on desktop */}
        <Logo className="select-none" />

        <nav className="flex items-center gap-1">
          <div className="hidden md:flex items-center gap-2 text-sm whitespace-nowrap overflow-x-auto no-scrollbar">
            {(navTabs ?? []).map((t) => {
        ...
        <div className="md:static absolute right-4 top-1/2 -translate-y-1/2 md:translate-y-0 md:top-auto flex items-center gap-2">
          <UserPill />
        </div>
      </div>
    </header>
