...
export async function POST(req) {
  ...
  try {
    ...
    const { jobId, source = null, anonId: bodyAnon = null, force = false } = body;
    if (!jobId) {
      return NextResponse.json({ error: "Missing jobId", stage }, { status: 400 });
    }
    ...
    const { error: insertError } = await sb.from("job_page_views").insert(payload);
    if (insertError) {
