...
function getSingleJobFallback() {
  return {
    heading: "Single job",
    selectedId: null,
    active: {
      id: null,
      title: "No open job available",
      company: "",
      location: "",
      service: "",
      description: "There are no live jobs to feature right now.",
      postedLabel: "",
      closeLabel: "",
      listingType: "",
      urgency: "",
      profileUrl: "/jobs",
      caption: "No open jobs are available to feature right now.",
    },
    options: [],
    byId: {},
  };
...
async function getSingleJobData(sb) {
  ...
  const options = jobs.map((job) => {
    const singleJob = {
      ...
      location: job.location || "Location flexible",
      service: job.service?.name || "General mining support",
      description: truncateText(job.description || "View this role on YouMine.", 240),
      postedLabel: job.created_at ? formatDateLabel(job.created_at, { day: "numeric", month: "short", year: "numeric" }) : "Recent",
      closeLabel: job.close_date ? formatDateLabel(job.close_date, { day: "numeric", month: "short", year: "numeric" }) : "Applications open",
      listingType: job.listing_type || "General",
      urgency: job.urgency || "Standard",
      profileUrl: `/jobs/${job.id}`,
      caption: [
        `${job.title || "Open role"}${job.company ? ` at ${job.company}` : ""}.`,
        `${job.service?.name || "General mining support"}${job.location ? ` • ${job.location}` : ""}.`,
        `View the role on youmine.io/jobs/${job.id}`,
      ].join("\n"),
    };

    byId[job.id] = singleJob;
...
export async function getSocialStudioData(sb) {
  ...
  const [consultantFeatureResult, jobsResult, singleJobResult, analyticsResult] = await Promise.allSettled([
  ...
  const singleJob = singleJobResult.status === "fulfilled"
    ? singleJobResult.value
    : {
        ...getSingleJobFallback(),
        active: {
          ...getSingleJobFallback().active,
          description: "Single job data could not be loaded.",
          caption: "Single job data could not be loaded.",
        },
      };

  if (singleJobResult.status === "rejected") {
    warnings.push(`Single job feature unavailable: ${singleJobResult.reason?.message || "Unknown error"}`);
  }
