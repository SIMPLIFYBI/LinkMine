import {
  ...
  RESOURCE_REQUEST_RESPONSE_STATUSES,
  RESOURCE_STATUSES,
  RESOURCE_TYPES,
...
export const RESOURCE_ALLOWED_IMAGE_EXTENSIONS = new Set([
  "jpg",
  "jpeg",
  "png",
  "webp",
...
export const RESOURCE_ALLOWED_IMAGE_MIME_TYPES = new Set([
  "image/jpeg",
  "image/png",
  "image/webp",
]);
...
export function isValidResourceRequestResponseStatus(value) {
  return RESOURCE_REQUEST_RESPONSE_STATUSES.includes(value);
}
...
export function isAllowedResourceImageUpload(file) {
  const extension = getFileExtension(file?.name || "");
  const mimeType = String(file?.type || "").toLowerCase();
  if (!RESOURCE_ALLOWED_IMAGE_EXTENSIONS.has(extension)) return false;
  if (!mimeType) return true;
  return RESOURCE_ALLOWED_IMAGE_MIME_TYPES.has(mimeType);
}
...
export function buildResourceRoutePayload(resourceRow, tagLinks = []) {
  ...
  return {
    ...
    claimContactEmail: resourceRow.claim_contact_email || null,
    ...
    openCount: Number(resourceRow.open_count ?? resourceRow.download_count ?? 0),
