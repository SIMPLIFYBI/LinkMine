...
export function buildClaimProfileHtml(consultantName, legacyUrl, token, enterCodeUrl) {
      ...
      <table role="presentation" cellspacing="0" cellpadding="0" border="0" align="center" width="100%" style="max-width:560px;margin:0 auto;">
        <tr><td style="padding:24px;">
          ...
          <div style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:16px;padding:20px;">
            ...
            <p style="margin:0 0 16px;font-size:13px;color:#9fb3c8;">Code expires in 30 minutes.</p>
            ...
            <p style="margin:0 0 6px;font-size:12px;color:#7dd3fc;">Legacy link (will be phased out):</p>
            <p style="margin:0;word-break:break-all;"><a href="${legacyUrl}" style="color:#7dd3fc;text-decoration:underline;">${legacyUrl}</a></p>
          </div>
...
export function buildClaimProfileText(consultantName, legacyUrl, token, enterCodeUrl) {
  const code = shortClaimCodeFromToken(token);
  return `Claim your profile: ${consultantName}

Claim code: ${code}
Enter code here: ${enterCodeUrl}

Legacy link (will be phased out):
${legacyUrl}
